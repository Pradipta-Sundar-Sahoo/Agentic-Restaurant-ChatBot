...
- Respond ONLY with the SQL query(s) or "END" followed by the answer. Do not include explanations unless it's part of the final answer after "END"."""
        ...
        if iteration > 0:
            ...
            # Check if there's an IMPORTANT note in the query suggesting previous results were insufficient
            insufficiency_note = ""
            if "IMPORTANT: The combined results so far have been judged INSUFFICIENT" in query:
                # Extract the insufficiency note
                parts = query.split("IMPORTANT:")
                original_query = parts[0].strip()
                insufficiency_details = parts[1].strip()
                
                # Add special instructions for handling insufficient results
                insufficiency_note = f"""
IMPORTANT: Previous results were judged INSUFFICIENT to answer the user's question completely.
{insufficiency_details}

Your task is to generate a SQL query that specifically addresses this gap in information.
Consider different search approaches than were used previously, such as:
1. Using fuzzy matching if exact matches returned no results
2. Broadening search criteria if they were too restrictive
3. Using different columns or joining tables differently
4. Applying different aggregations or sorting methods
"""
                # Update the query to just use the original part
...
async def generate_pinecone_query(state: AgentState) -> AgentState:
    ...
    # Generate vector query with context judgment reasoning if available
    if hasattr(state, "reasoning") and state.reasoning:
        logger.info(f"Using context judgment reasoning for vector search: {state.reasoning}")
        query_params = await _generate_vector_query_with_reasoning(state)
    else:
...
async def _generate_vector_query_with_reasoning(state: AgentState) -> Dict[str, Any]:
    """Generate Pinecone query parameters based on user query and reasoning from context judgment"""
    logger.info("Generating vector search parameters with context judgment reasoning")
...
IMPORTANT: Previous search results were deemed INSUFFICIENT. You need to create a new vector search that will address the following gap in information:
{state.reasoning}
...
IMPORTANT: 
- Create a search that will specifically address the missing information identified above
- Use fewer strict metadata filters and rely more on query_text for semantic matching
...
{{"query_text": "spicy chicken dishes", "metadata_filters": {{"item_is_veg": false}}}}"""
    ...
    messages = [
        ...
        {"role": "user", "content": f"Generate vector search parameters for this query based on identified gaps: {state.user_query}"}
    ]
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Generated vector search parameters with reasoning: {query_params}")
            return query_params
        except json.JSONDecodeError:
            logger.error(f"Error parsing vector search parameters JSON: {cleaned_response}")
            # Fallback to using the raw query
            return {"query_text": state.reasoning, "metadata_filters": {}}
            
    except Exception as e:
        logger.error(f"Error generating vector search parameters with reasoning: {str(e)}")
        return {"query_text": state.reasoning, "metadata_filters": {}}
...
def decide_next_step_after_judgment(state: AgentState) -> Literal["generate_final_response", "generate_queries", "generate_pinecone_query"]:
    """
    Decide route after context judgment based on the judgment's conclusion.
    - If state is marked complete (sufficient context), go to final response.
    - If more SQL is recommended, go to SQL generation.
    - If more Pinecone search is recommended, go to Pinecone generation.
    - Default to final response if uncertain.
    """
    if state.is_complete:
        logger.info("Routing: Context deemed sufficient or max iterations reached. Proceeding to final response.")
        return "generate_final_response"
    
    # Get recommended search method from the judgment
    recommended_method = state.context_judgment.get("recommended_search_method", "none")
    
    if recommended_method == "sql":
        logger.info("Routing: Context judge recommends more SQL queries. Proceeding to SQL generation.")
        return "generate_queries"
    elif recommended_method == "pinecone":
        logger.info("Routing: Context judge recommends more vector search. Proceeding to Pinecone query generation.")
        return "generate_pinecone_query"
    elif recommended_method == "both":
        # Start with SQL for 'both' since that's the typical flow
        logger.info("Routing: Context judge recommends both SQL and Pinecone. Starting with SQL generation.")
        return "generate_queries"
    else:
        # Default to final response if no clear recommendation
        logger.info("Routing: No clear search recommendation from context judge. Proceeding to final response.")
        state.is_complete = True  # Mark as complete since we're ending
        return "generate_final_response"
...
async def generate_queries(state: AgentState) -> AgentState:
    ...
    # Check if we have reasoning from context judgment to include in the prompt
    additional_context = ""
    if hasattr(state, "reasoning") and state.reasoning:
        additional_context = f"\n\nIMPORTANT: The combined results so far have been judged INSUFFICIENT. More information is needed about: {state.reasoning}\n\nPlease generate a SQL query to address this specific gap in information."
        logger.info(f"Adding context judgment reasoning to SQL generation: {state.reasoning}")
    
    llm_response = llm_client.generate_sql(
        query=state.user_query + additional_context,
...
async def combine_contexts(state: AgentState) -> AgentState:
    ...
    # Only combine if we have new results to add
    has_new_sql_results = len(state.sql_results) > 0 and state.search_decision.get("sql", 0) == 1
    has_new_pinecone_results = len(state.pinecone_results) > 0 and state.search_decision.get("pinecone", 0) == 1
    
    if not has_new_sql_results and not has_new_pinecone_results:
        logger.warning("No new results to combine")
        return state
    
    # Create a combined context string
    combined_context = "Combined Search Results:\n\n"
    
    # If we already had a previous combined context from a previous iteration after judgment, 
    # include that information about what was missing
    previous_reasoning = ""
    if hasattr(state, "reasoning") and state.reasoning:
        previous_reasoning = f"PREVIOUS ITERATION NOTE: The prior search results were judged insufficient. Missing information was about: {state.reasoning}\n\n"
        combined_context += previous_reasoning
    
    # Add information about the original query for context
    combined_context += f"Original User Query: {state.user_query}\n\n"
    combined_context += f"Current Iteration: {state.current_iteration}\n\n"
    
    # Add information about the search strategy used
    combined_context += "Search Strategy Used:\n"
    if state.search_decision.get("sql", 0) == 1:
        combined_context += "- SQL Database Search\n"
    if state.search_decision.get("pinecone", 0) == 1:
        combined_context += "- Vector Database Search\n"
    if state.search_decision.get("chat_history_lookup", 0) == 1:
        combined_context += "- Chat History Analysis\n"
    combined_context += "\n"
    ...
    if state.sql_results:
        ...
        for i, result in enumerate(state.sql_results):
            ...
            # Add assessment of results
            if len(result) == 0:
                combined_context += "Assessment: No results found for this query. Consider alternative search approaches.\n\n"
            elif "error" in result[0]:
                combined_context += f"Assessment: Error encountered: {result[0].get('error', 'Unknown error')}\n\n"
            else:
                combined_context += f"Assessment: Query returned {len(result)} results.\n\n"
    else:
    ...
    if state.pinecone_results:
        ...
        for i, result in enumerate(state.pinecone_results):
            ...
            if "results" in result and "matches" in result["results"]:
                ...
                # Add assessment of vector search results
                if len(matches) == 0:
                    combined_context += "Assessment: No matches found for this vector search. Consider refining search terms.\n\n"
                else:
                    # Calculate average similarity score
                    avg_score = sum(match.get("score", 0) for match in matches) / len(matches) if matches else 0
                    combined_context += f"Assessment: Found {len(matches)} matches with average similarity score of {avg_score:.4f}.\n\n"
                    
                    # Suggest if scores are low
                    if avg_score < 0.7 and len(matches) > 0:
                        combined_context += "Note: Similarity scores are relatively low. Results may not be highly relevant to the query.\n\n"
            elif "error" in result:
                combined_context += f"Error: {result.get('error', 'Unknown error')}\n\n"
            else:
                combined_context += "No matches found in vector search.\n\n"
    else:
        combined_context += "== Vector Database Results ==\nNo vector searches were executed.\n\n"
    
    # Add suggestions for potential further searches
    combined_context += "== Potential Additional Search Strategies ==\n"
    
    # If no SQL queries were performed but might be useful
    if not state.sql_results and not (state.search_decision.get("sql", 0) == 1):
        combined_context += "- SQL search could provide structured information about specific menu items, prices, or restaurants.\n"
    
    # If no Pinecone searches were performed but might be useful
    if not state.pinecone_results and not (state.search_decision.get("pinecone", 0) == 1):
        combined_context += "- Vector search could provide semantic matches for food descriptions or conceptually similar menu items.\n"
    
    # If existing searches returned limited results
    if state.sql_results and all(len(result) == 0 for result in state.sql_results):
        combined_context += "- Previous SQL queries returned no results. Consider using fuzzy matching or broader terms.\n"
    
    if state.pinecone_results and all(
        not result.get("results", {}).get("matches", []) for result in state.pinecone_results
    ):
        combined_context += "- Previous vector searches returned no matches. Consider using more general terms or fewer filters.\n"
...
5. Whether the current contexts are SUFFICIENT to provide a complete answer to the user's query
...
- "is_sufficient": Boolean indicating whether the context is sufficient to answer the query
- "missing_information": If is_sufficient is false, describe what additional information is needed
- "recommended_search_method": If is_sufficient is false, recommend which search method to use: "sql", "pinecone", "both", or "none"

This assessment will be used to determine whether to continue searching or generate a final response."""
    
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": f"User Query: {state.user_query}\n\nCombined Context:\n{state.combined_context}\n\nPlease evaluate this combined information and determine if it's sufficient to answer the user's query."}
    ]
    ...
    try:
        ...
        is_sufficient = judgment.get("is_sufficient", True)
        if not is_sufficient and state.current_iteration < 5:  # Limit additional iterations
            logger.info(f"Context deemed insufficient. Missing: {judgment.get('missing_information', 'Not specified')}")
            
            # Update search decision based on recommended method
            recommended_method = judgment.get("recommended_search_method", "none")
            if recommended_method in ["sql", "pinecone", "both"]:
                # Update the search decision to continue with the recommended method
                if recommended_method == "sql":
                    state.search_decision = {"sql": 1, "pinecone": 0, "chat_history_lookup": 0, "conversational": 0}
                elif recommended_method == "pinecone":
                    state.search_decision = {"sql": 0, "pinecone": 1, "chat_history_lookup": 0, "conversational": 0}
                elif recommended_method == "both":
                    state.search_decision = {"sql": 1, "pinecone": 1, "chat_history_lookup": 0, "conversational": 0}
                
                # Add reasoning for the next iteration
                state.reasoning = judgment.get("missing_information", "Additional information needed.")
                
                # Mark as not complete to continue the loop
                state.is_complete = False
                logger.info(f"Will continue with additional {recommended_method} search")
            else:
                # If no clear recommendation, just proceed to final response
                logger.info("No clear recommendation for additional search method. Proceeding to final response.")
                state.is_complete = True
        else:
            # Context is sufficient or max iterations reached
            if not is_sufficient and state.current_iteration >= 5:
                logger.warning("Max iterations reached. Proceeding to final response despite insufficient context.")
            else:
                logger.info("Context judged as sufficient. Proceeding to final response.")
            state.is_complete = True
        ...
        state.is_complete = True  # Proceed to final response on error
        return state
...
4. Conversational Response - Good for:
   - Casual greetings ("hello", "hi", "good morning")
   - Personal questions about the assistant ("what's your name", "who are you")
   - General food questions that don't require specific database lookups
   - Small talk and pleasantries

IMPORTANT PERSONALITY NOTES:
- The assistant's name is Nugget
- Nugget is friendly, helpful, and knowledgeable about restaurants
- Nugget should always respond gracefully to any query, even if it's not restaurant-related
- For casual greetings or personal questions, set "conversational": 1 in your decision
...
- Conversational only (conversational=1, others=0) - For greetings and general conversation
- Any combination - When a comprehensive approach is needed
- None (all=0) - When the query is outside the scope of restaurant data (avoid this if possible)
...
- "conversational": 0 or 1 (whether this is just casual conversation)
...
- "suggested_response": If conversational=1, suggest a friendly response as Nugget
...
Consider the query structure, implied search needs, potential results, and prior conversation context."""
    ...
    try:
        ...
        if "sql" not in decision or "pinecone" not in decision:
            ...
            decision = {"sql": 1, "pinecone": 0, "chat_history_lookup": 0, "conversational": 0, "explanation": "Default to SQL due to incomplete judge response"}
        ...
        decision["conversational"] = 1 if decision.get("conversational") else 0
        ...
        # Ensure suggested_response is included if conversational is used
        if decision["conversational"] == 1 and "suggested_response" not in decision:
            decision["suggested_response"] = "Hi there! I'm Nugget, your restaurant assistant. How can I help you find delicious food today?"
            logger.warning("Judge set conversational=1 but didn't provide suggested_response")
        
        # Store the decision
        state.search_decision = decision
        logger.info(f"Judge decision: SQL={decision['sql']}, Pinecone={decision['pinecone']}, Chat History Lookup={decision['chat_history_lookup']}, Conversational={decision['conversational']}, Reason: {decision.get('explanation', 'No explanation provided')}")
        
        # If all search methods are deemed unnecessary but not conversational, make it conversational
        if decision["sql"] == 0 and decision["pinecone"] == 0 and decision["chat_history_lookup"] == 0 and decision["conversational"] == 0:
            logger.warning("Judge found no suitable method, defaulting to conversational")
            state.search_decision["conversational"] = 1
            state.search_decision["suggested_response"] = "Hi there! I'm Nugget, your restaurant assistant. I'd be happy to help you find information about restaurants, menu items, or answer food-related questions. What would you like to know?"
...
Keep your response focused on the restaurant/menu information that was previously discussed."""
    ...
    try:
        ...
        # Fallback response
        state.final_response = "I'm sorry, I couldn't properly process the previous conversation history to answer your question."
        state.is_complete = True
        return state

async def generate_conversational_response(state: AgentState) -> AgentState:
    """Generate a conversational response for casual queries and greetings"""
    logger.info("Generating conversational response")
    
    # Check if judge provided a suggested response
    suggested_response = state.search_decision.get("suggested_response", "")
    
    if suggested_response:
        logger.info(f"Using judge's suggested response: {suggested_response[:50]}...")
        state.final_response = suggested_response
        state.is_complete = True
        return state
    
    # If no suggested response, generate one with the LLM
    system_prompt = """You are Nugget, a friendly AI assistant that helps users find information about restaurants and menu items.

When responding to casual greetings, introductions, or general questions:
- Be warm, friendly, and conversational
- Mention your name (Nugget) appropriately
- Encourage users to ask about restaurants, menu items, or food
- Respond gracefully to any question, even if it's not directly about restaurants
- Keep responses concise but personable

Your specialty is helping users find restaurant information, recommend dishes, and answer food-related questions."""
    ...
    if state.chat_history and len(state.chat_history) > 0:
        chat_history_context = "\nPrevious conversation context:\n"
        # Get the last 3 messages maximum
    ...
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": f"User message: {user_query}{chat_history_context}\n\nPlease respond to this casual message as Nugget."}
    ]
    ...
    try:
        ...
        # Set the final response
        state.final_response = response.strip()
        logger.info(f"Generated conversational response: {state.final_response[:50]}...")
        ...
        # Fallback response
        state.final_response = "Hi there! I'm Nugget, your restaurant assistant. How can I help you find delicious food today?"
        state.is_complete = True
        return state

# Route based on judge decision
def route_based_on_decision(state: AgentState) -> Literal["sql_only", "pinecone_only", "chat_history_only", "conversational_only", "sql_and_chat", "pinecone_and_chat", "both", "all_three", "none"]:
...
def build_agent():
    ...
    # Add the new node for generating conversational responses
    ...
    workflow.add_conditional_edges(
        ...
        route_based_on_decision,
        {
            ...
            "conversational_only": "generate_conversational_response",  # Direct to conversational response
            "sql_and_chat": "generate_queries",
            "pinecone_and_chat": "generate_pinecone_query",
            "both": "generate_queries",
            "all_three": "generate_queries",
            "none": "generate_conversational_response"  # Route "none" responses to conversational instead of ending
        }
    )
    ...
    # Add edge from conversational response to END
    ...
    # Add the conditional routing from judge_combined_context
    workflow.add_conditional_edges(
        "combine_contexts", 
        lambda state: "judge_combined_context", 
        {"judge_combined_context": "judge_combined_context"}
    )
    
    # Add the new conditional routing from judge_combined_context
    workflow.add_conditional_edges(
        "judge_combined_context",
        decide_next_step_after_judgment,
        {
            "generate_final_response": "generate_final_response",
            "generate_queries": "generate_queries",
            "generate_pinecone_query": "generate_pinecone_query"
        }
    )
